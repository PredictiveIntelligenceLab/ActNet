...
def get_value_and_laplacian(f):
    '''
    Computes the 2D laplacian of a given funciton in a computationally efficient
    way by avoiding unecessary repetition of the computational graph.

    Args:
        f (Callable): a function with signature (params, x, y)->scalar
    Returns:
        (Callable): a function with signature (params, x, y)->scalar, scalar that is the
                    laplacian of input function
    '''
    _single_f = lambda params, x, y : f(params, x[None,:], y[None,:]).squeeze()
    def _scalar_grads(params, x, y):
        u, (ux, uy) = value_and_grad(_single_f, argnums=(1,2))(params, x, y)
        return ux.squeeze(), (u, uy.squeeze())
    def _lapl_aux(params, x, y):
        (u_x, (u, u_y)), u_xx =  value_and_grad(_scalar_grads,
                                                argnums=1,
                                                has_aux=True)(params, x, y)
        return u_y, (u, u_xx.squeeze())
    def _lapl_aux_2(params, x, y):
        (u_y, (u, u_xx)), u_yy =  value_and_grad(_lapl_aux,
                                                 argnums=2,
                                                 has_aux=True)(params, x, y)
        return u, u_xx, u_yy.squeeze()
    def _value_and_lapl(params, xs, ys):
        u, uxx, uyy = vmap(_lapl_aux_2, in_axes=(None, 0, 0))(params, xs, ys)
        return u, uxx + uyy
    return _value_and_lapl


class HelmholtzModel:
    """ A model for training/evaluating a neural network using physics-informed
    training on the 2D Helmholtz problem.
    ...
    def __init__(self, arch, batch, true_fun=None,
                 ...
                 pinn_weights=(0.001, 1.), exact_bd_condition=False,
               ...
               _apply = lambda params, x, y : \
                (1-x**self.bdr_enforcer_order)\
                    *(1-y**self.bdr_enforcer_order)\
                        *self.arch.apply(params,
                                         (jnp.hstack([x, y])-mu_x)/sig_x)
            else:
               _apply = lambda params, x, y : \
                self.arch.apply(params,
                                (jnp.hstack([x, y])-mu_x)/sig_x)
        ...
        # laplacian
        self.value_and_laplacian = get_value_and_laplacian(self.apply)
    ...
    def residual_loss(self, params, x, y):
        ...
        u, delta_u = self.value_and_laplacian(params, x, y)
        u, delta_u = u[:,None], delta_u[:,None]
        res = delta_u + (self.kappa**2)*u
